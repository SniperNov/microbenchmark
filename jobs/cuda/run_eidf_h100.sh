#!/bin/bash
# EIDF H100 CUDA run script for VDI sessions.

set -e

OUTDIR="result/EIDF_H100_Output/CUDA"
mkdir -p "$OUTDIR"

JOB_NAME="eidf_h100_cuda"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTFILE="$OUTDIR/${JOB_NAME}_${TIMESTAMP}.out"

echo "Running EIDF H100 CUDA benchmark groups..." | tee "$OUTFILE"
...
echo "========== EIDF H100 CUDA configuration ==========" | tee -a "$OUTFILE"
...
echo "" | tee -a "$OUTFILE"
echo "All EIDF H100 CUDA benchmark groups completed." | tee -a "$OUTFILE"
