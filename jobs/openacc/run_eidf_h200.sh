#!/bin/bash
# EIDF H200 OpenACC run script for VDI sessions.

set -e

OUTDIR="result/EIDF_H200_Output/OpenACC"
mkdir -p "$OUTDIR"

JOB_NAME="eidf_h200_openacc"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTFILE="$OUTDIR/${JOB_NAME}_${TIMESTAMP}.out"

echo "Running EIDF H200 OpenACC benchmark groups..." | tee "$OUTFILE"
...
echo "========== EIDF H200 OpenACC configuration ==========" | tee -a "$OUTFILE"
...
echo "" | tee -a "$OUTFILE"
echo "All EIDF H200 OpenACC benchmark groups completed." | tee -a "$OUTFILE"
