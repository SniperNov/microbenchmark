#!/bin/bash
# EIDF A100 OpenACC run script for VDI sessions.

set -e

OUTDIR="result/EIDF_A100_Output/OpenACC"
mkdir -p "$OUTDIR"

JOB_NAME="eidf_a100_openacc"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTFILE="$OUTDIR/${JOB_NAME}_${TIMESTAMP}.out"

echo "Running EIDF A100 OpenACC benchmark groups..." | tee "$OUTFILE"
...
echo "========== EIDF A100 OpenACC configuration ==========" | tee -a "$OUTFILE"
...
echo "" | tee -a "$OUTFILE"
echo "All EIDF A100 OpenACC benchmark groups completed." | tee -a "$OUTFILE"
