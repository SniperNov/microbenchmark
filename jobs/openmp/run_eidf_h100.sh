...
OUTDIR="result/EIDF_H100_Output/OpenMP"
mkdir -p "$OUTDIR"

JOB_NAME="eidf_h100_omp"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTFILE="$OUTDIR/${JOB_NAME}_${TIMESTAMP}.out"

echo "Running EIDF H100 OpenMP benchmark groups..." | tee "$OUTFILE"
...
echo "SET UP H100 configuration= NVIDIA H100-80GB-HBM3 ========" | tee -a "$OUTFILE"
...
echo "" | tee -a "$OUTFILE"
echo "All EIDF H100 benchmark groups completed." | tee -a "$OUTFILE"
