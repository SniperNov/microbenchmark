...
def load_distribution_csv(path: str) -> pd.DataFrame:
    ...
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        ...
        config_a_name = "thread_count"
        config_b_name = "team_count"
        if header and len(header) >= 5:
            config_a_name = header[3].strip() or config_a_name
            config_b_name = header[4].strip() or config_b_name
...
def load_overhead_txt(path: str) -> pd.DataFrame:
    ...
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            m2 = re.search(
                r"Set=(\d+)\s+Run=(\d+)\s+Lmin=([-\d\.]+)\s+"
                r"Intercept=([-\d\.]+)\s*(?:μs|us)\s+"
                r"Slope=([-\d\.]+)",
                line
            )
                
            if m2 and current:
                lowest_match = re.search(r"Lowest=([-\d\.]+)\s*(?:μs|us)", line)
                rows.append({
                    **current,
                    "set": int(m2.group(1)),
                    "run": int(m2.group(2)),
...
def plot(dist: pd.DataFrame, ovh: pd.DataFrame, out_png: str, scale: str, delay_spec):
    ...
    if len(config_columns) >= 2:
        cfg_a_name, cfg_b_name = config_columns[0], config_columns[1]
    else:
        cfg_a_name, cfg_b_name = dist.columns[3], dist.columns[4]

    cfg_a = int(dist[cfg_a_name].iloc[0])
    cfg_b = int(dist[cfg_b_name].iloc[0])
    ...
    ax.set_title(
        f"Exec time vs delaylength — Method {method_id}, N={N}, "
        f"{cfg_a_name}={cfg_a}, {cfg_b_name}={cfg_b}"
    )
...
def main():
    ...
    out_png = sys.argv[5] if len(sys.argv) >= 6 else f"4colorDist{delay_spec or 'all'}.png"
